...
export default function Page() {
  ...
  const saveResponse = async () => {
    ...
    try {
      ...
      setComplaint((prev) =>
        prev ? { ...prev, response: response.trim(), status: 'Resolved' } : prev
      );
    } finally {
      setSaving(false);
    }
  ...
  const rejectComplaint = async () => {
    ...
    try {
      ...
      setComplaint((prev) =>
        prev ? { ...prev, status: 'Rejected', rejectionReason: rejectionReason.trim() || 'No reason provided' } : prev
      );
    } finally {
      setSaving(false);
    }
    ...
    <div className="mx-auto max-w-4xl space-y-6 p-6">
      ...
      <div className="rounded-xl border bg-white p-6 shadow">
        ...
        <div className="mt-8 border-t pt-6">
          <h2 className="text-lg font-semibold text-slate-900">Reject Complaint</h2>

          <textarea
            value={rejectionReason}
            onChange={(e) => setRejectionReason(e.target.value)}
            className="mt-4 min-h-[120px] w-full rounded-lg border border-slate-300 p-3 outline-none focus:border-emerald-600"
